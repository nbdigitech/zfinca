...
<div class="content">
  ...
  <div class="row">
        <div class="col-md-12">
              <form id="TypeValidation" class="form-horizontal" action="@if(isset($edit->id)) {{route('Admin/TeamMember/Update')}} @else {{route('Admin/TeamMember/Store')}} @endif" method="post" enctype="multipart/form-data">
                {{csrf_field()}}
               <input type="hidden" name="Update" value="@if(isset($edit->id)) {{$edit->id}} @endif">
                <div class="card ">
                  ...
                  <div style="background-color: #303948; padding: 10px; padding-left: 16px; padding-top:16px; border-radius: 3px;" style="width: 100%;">
                    <h4>
                      @if(isset($edit->id)) Update TeamMember @else Create TeamMember @endif                                       
                       <a href="{{route('Admin/TeamMember')}}" class="btn btn-success pull-right" style="margin-top: -7px; background-color: white; color:black; box-shadow:none;">Manage</a></h4>
                  </div>
                </div>
                  <div class="card-body">
                ...
                @endif
                   ...
                   <div class="row" style="margin-bottom: 30px;">
                      ...
                      <div class="col-sm-4">
                        <div class="form-group">
                           <input type="text" required name="Post" value="@if(isset($edit->id)) {{$edit->Post}} @endif" class="form-control">
                        </div>
                      </div>
                  </div>
  ...
  <script type="text/javascript">
      $(document).ready(function(){
        ...
        $("#mastercategory").change(function(e){
          ...
          $.ajax({
            type:"post",
            url:"{{url('Admin/MasterCategory/TeamMember/MCChange')}}",
            data:{id:mastercategory},
            success:function(data){
              if(data!=''){
              $("#mastersubcategory").empty().append(data);
              $("#subcategory").empty().append('<option value="">Select TeamMember SubCategory</option>');
              }
              else{
                $("#mastersubcategory").empty().append('<option value="">Select TeamMember Category</option>');
                $("#subcategory").empty().append('<option value="">Select TeamMember SubCategory</option>');
              }
            }
          });
  ...
  <script type="text/javascript">
     $(document).ready(function(){
         $("#mastersubcategory").on('change',function(e){
          ...
          $.ajax({
            type:"post",
            url:"{{url('Admin/MasterCategory/TeamMember/MSCChange')}}",
